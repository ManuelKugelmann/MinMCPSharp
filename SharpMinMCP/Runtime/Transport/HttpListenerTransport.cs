    ...
    public class HttpListenerTransport : IMcpTransport
    {
        ...
        private void ListenLoop()
        {
            while (IsRunning && !_cts.IsCancellationRequested)
            {
                ...
                // Fire-and-forget each request (thread pool)
                var captured = ctx;
        ...
        private bool ValidateOrigin(HttpListenerRequest req)
        {
            ...
            if (string.IsNullOrEmpty(origin)) return true; // no origin = same-origin
            ...
            // Default: allow localhost variants
